...
install:
	CGO_ENABLED=1 $(GO) install -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS)
...
build-deps:
	for dep in $$(grep -E "(sylr/quickfixgo|quickfixgo)" go.mod | awk '{ print $$1; }'); do \
	  CGO_ENABLED=1 GOOS=$(GOOS) GOARCH=$(GOARCH) GOARM=$(GOARM) $(GO) build -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS) $$dep; \
	done

$(GO_BUILD_VERSION_TARGET): $(GO_BUILD_SRC) $(GO_GENERATE_TARGET) $(GO_BUILD_FLAGS_TARGET) | $(GO_BUILD_TARGET_DEPS)
	CGO_ENABLED=1 GOOS=$(GOOS) GOARCH=$(GOARCH) GOARM=$(GOARM) $(GO) build -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS) -o $@
...
$(GO_CROSSBUILD_WINDOWS_TARGET_PATTERN): $(GO_BUILD_SRC) $(GO_BUILD_FLAGS_TARGET)
	CGO_ENABLED=1 GOOS=windows GOARCH=$* $(GO) build -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS) -o $@

$(GO_CROSSBUILD_386_TARGET_PATTERN): $(GO_BUILD_SRC) $(GO_BUILD_FLAGS_TARGET)
	CGO_ENABLED=1 GOOS=$* GOARCH=386 $(GO) build -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS) -o $@

$(GO_CROSSBUILD_AMD64_TARGET_PATTERN): $(GO_BUILD_SRC) $(GO_BUILD_FLAGS_TARGET)
	CGO_ENABLED=1 GOOS=$* GOARCH=amd64 $(GO) build -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS) -o $@

$(GO_CROSSBUILD_ARM_TARGET_PATTERN): $(GO_BUILD_SRC) $(GO_BUILD_FLAGS_TARGET)
	CGO_ENABLED=1 GOOS=$* GOARCH=arm $(GO) build -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS) -o $@

$(GO_CROSSBUILD_ARM64_TARGET_PATTERN): $(GO_BUILD_SRC) $(GO_BUILD_FLAGS_TARGET)
	CGO_ENABLED=1 GOOS=$* GOARCH=arm64 $(GO) build -tags "$(GO_BUILD_TAGS)" $(GO_BUILD_FLAGS) $(GO_BUILD_LDFLAGS) -o $@
